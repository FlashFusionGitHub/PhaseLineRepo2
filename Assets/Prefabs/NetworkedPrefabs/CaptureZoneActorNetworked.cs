...
public class CaptureZoneActorNetworked : MonoBehaviour {
    ...
    public List<TroopActorNetworked> team1unitsInZone;
    public List<TroopActorNetworked> team2unitsInZone;
    ...
    private void OnTriggerExit(Collider other)
    {
        ...
        if (currentActor)
        {
            if (other.GetComponent<TroopActorNetworked>().team == Team.TEAM1)
            {
                team1unitsInZone.Remove(other.GetComponent<TroopActorNetworked>());
            }

            if (other.GetComponent<TroopActor>().team == Team.TEAM2)
            {
                team2unitsInZone.Remove(other.GetComponent<TroopActorNetworked>());
            }
        }
    }
}
